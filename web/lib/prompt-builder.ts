...
export function buildPrompt(simId: string, themeId: string): string {
  ...
  const themeBase = fs.readFileSync(paths.THEME_BASE, 'utf8');
  ...
  const themeContent = themeFile.replace(/^---\n[\s\S]*?\n---\n/, '');
  ...
  body = body
    ...
    .replace('{themes/_base.md contents}', themeBase)
    .replace('{themes/{theme_id}.md contents, frontmatter stripped}', themeContent);
