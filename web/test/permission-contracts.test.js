...
describe('web app tool whitelist', () => {
  ...
  it('has exactly 3 allowedTools occurrences, each restricted to Read and Write', () => {
    const matches = [];
    ...
    assert.equal(matches.length, 3,
      'expected exactly 3 allowedTools declarations, found ' + matches.length);
  ...
  it('has exactly 3 permissionMode occurrences, each set to bypassPermissions', () => {
    const matches = [];
    for (let i = 0; i < lines.length; i++) {
    ...
    assert.equal(matches.length, 3,
      'expected exactly 3 permissionMode declarations, found ' + matches.length);
    for (const m of matches) {
      assert.ok(
