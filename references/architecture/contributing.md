...
1. Read `references/operations/web-app-checklist.md` and check every relevant item
2. Read `references/architecture/workspace-map.md` to understand data flow and impact
...
- Architecture reference: `references/architecture/testing-system.md`
...
Paths in backticks (`.md`) and string values (`.json`) are tracked by `scripts/extract_paths.py`, which writes `references/registries/path-registry.csv`. Run `npm test` to regenerate the CSV and validate all paths resolve on disk. The test (`web/test/path-registry.test.ts`) checks: concrete paths exist, template paths have valid directory prefixes, source files exist.
